...
static float BernsteinDegree4(int i, float t)
{
    switch (i)
    {
...
static ::DirectX::SimpleMath::Vector3 EvaluateBiQuarticBezierPatch(const ::DirectX::SimpleMath::Vector3 control[5][5], float u, float v)
{
    ::DirectX::SimpleMath::Vector3 result(0.0f, 0.0f, 0.0f);
    for (int i = 0; i < 5; ++i)
    {
        float bu = BernsteinDegree4(i, u);
        for (int j = 0; j < 5; ++j)
        {
            float bv = BernsteinDegree4(j, v);
...
static ::std::vector<::DirectX::SimpleMath::Vector3> SimulateTessellationForPatch(const ::std::vector<::DirectX::SimpleMath::Vector3>& patchControlPoints)
{
    const int tessFactor = 16; // 고정 테셀레이션 팩터: 16분할 → 17×17 정점
    const int gridSize = tessFactor + 1; // 17

    ::DirectX::SimpleMath::Vector3 bezier[5][5];
    for (int i = 0; i < 5; ++i)
    {
    ...
    ::std::vector<::DirectX::SimpleMath::Vector3> tessellated;
    ...
    for (int i = 0; i <= tessFactor; ++i)
    {
        ...
        for (int j = 0; j <= tessFactor; ++j)
        {
            ...
            ::DirectX::SimpleMath::Vector3 pos = EvaluateBiQuarticBezierPatch(bezier, u, v);
...
bool SimulateTessellationAndWriteFile(const ::std::filesystem::path& heightMapPath,
    const ::std::filesystem::path& outputFilePath)
{
    ...
    ::std::size_t numPatches = meshData.position.size() / static_cast<::std::size_t>(patchControlPointCount);
    ...
    for (::std::size_t patchIndex = 0; patchIndex < numPatches; ++patchIndex)
    {
        auto patchBegin = meshData.position.begin() + patchIndex * patchControlPointCount;
        ::std::vector<::DirectX::SimpleMath::Vector3> controlPoints(patchBegin, patchBegin + patchControlPointCount);

        // CPU에서 테셀레이션 시뮬레이션: 5×5 컨트롤 포인트 전부를 사용하여 (17×17) tessellated vertices 생성
        ::std::vector<::DirectX::SimpleMath::Vector3> tessVertices = SimulateTessellationForPatch(controlPoints);
    ...
    for (const auto& patch : patches)
    {
        outFile.write(reinterpret_cast<const char*>(patch.vertices.data()),
            patch.vertices.size() * sizeof(::DirectX::SimpleMath::Vector3));
    }
