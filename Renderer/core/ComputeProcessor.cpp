...
MotionBlurProcessor::MotionBlurProcessor(ComPtr<ID3D12Device> device) {

}

void MotionBlurProcessor::CreateShader(ComPtr<ID3D12Device> device) {
	ComputeProcessor::CreateShader(device);

}

void MotionBlurProcessor::RegisterTexture(ComPtr<ID3D12Device> device, Texture& texture) {
	D3D12_SHADER_RESOURCE_VIEW_DESC srvDesc{};
	srvDesc.Format = DXGI_FORMAT_R32G32B32A32_FLOAT;
	srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D;
	srvDesc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
	...
	CD3DX12_CPU_DESCRIPTOR_HANDLE blurHandle(mHeap->GetCPUDescriptorHandleForHeapStart());
	blurHandle.Offset(2, device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV));
	device->CreateShaderResourceView(texture.GetResource().Get(), &srvDesc, blurHandle);
}

void MotionBlurProcessor::Dispatch(ComPtr<ID3D12Device> device, ComPtr<ID3D12GraphicsCommandList> commandList, Texture* input, Texture* output) {
	mCopyMap.Transition(commandList, D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_COPY_DEST);
	commandList->CopyResource(mCopyMap.GetResource().Get(), input->GetResource().Get());
	mCopyMap.Transition(commandList, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_ALL_SHADER_RESOURCE);
	mComputeMap.Transition(commandList, D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
	...
	gpuHandle.Offset(1, device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV));
	commandList->SetComputeRootDescriptorTable(2, gpuHandle); //srv

	UINT numGroupsX = static_cast<UINT>(std::ceilf(Config::WINDOW_WIDTH<UINT> / 16.0f));
	UINT numGroupsY = static_cast<UINT>(std::ceilf(Config::WINDOW_WIDTH<UINT> / 16.0f));

	commandList->Dispatch(numGroupsX, numGroupsY, 1);

	mCopyMap.Transition(commandList, D3D12_RESOURCE_STATE_ALL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_COMMON);
	input->Transition(commandList, D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_COPY_DEST);
	mComputeMap.Transition(commandList, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_COPY_SOURCE);
	commandList->CopyResource(input->GetResource().Get(), mComputeMap.GetResource().Get());
	mComputeMap.Transition(commandList, D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_COMMON);

}

void MotionBlurProcessor::CreateResource(ComPtr<ID3D12Device> device) {
	mComputeMap = Texture(device, DXGI_FORMAT_R8G8B8A8_UNORM, Config::WINDOW_WIDTH<UINT64>, Config::WINDOW_HEIGHT<UINT>, D3D12_HEAP_FLAG_NONE, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
	mCopyMap = Texture(device, DXGI_FORMAT_R8G8B8A8_UNORM, Config::WINDOW_WIDTH<UINT64>, Config::WINDOW_HEIGHT<UINT>, D3D12_HEAP_FLAG_NONE, D3D12_RESOURCE_FLAG_NONE);

}

void MotionBlurProcessor::CreateHeap(ComPtr<ID3D12Device> device) {
	D3D12_DESCRIPTOR_HEAP_DESC blurDesc{};
	blurDesc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
	blurDesc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
	blurDesc.NumDescriptors = 3;
	blurDesc.NodeMask = 0;
	device->CreateDescriptorHeap(&blurDesc, IID_PPV_ARGS(mHeap.GetAddressOf()));
}

void MotionBlurProcessor::CreateView(ComPtr<ID3D12Device> device) {
	CD3DX12_CPU_DESCRIPTOR_HANDLE blurHandle(mHeap->GetCPUDescriptorHandleForHeapStart());
	...
	blurHandle.Offset(1, device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV));
	device->CreateShaderResourceView(mCopyMap.GetResource().Get(), &srvDesc, blurHandle);
}

void MotionBlurProcessor::CompileShader() {
	std::wstring filename = L"Shader/Sources/MotionBlur.hlsl";
	...
	HRESULT hr = D3DCompileFromFile(filename.c_str(), nullptr, nullptr, "MotionBlur_CS", "cs_5_1", flags, 0, mShaderCode.GetAddressOf(), mError.GetAddressOf());
...
void MotionBlurProcessor::CreateRootSignature(ComPtr<ID3D12Device> device) {
	CD3DX12_DESCRIPTOR_RANGE uavTable;
	uavTable.Init(D3D12_DESCRIPTOR_RANGE_TYPE_UAV, 1, 0);
	...
	rootParameter[2].InitAsDescriptorTable(1, &srvTable2);

	CD3DX12_ROOT_SIGNATURE_DESC rootSignatureDesc(3, rootParameter,
		0, nullptr,
		D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT);
...
void MotionBlurProcessor::CreatePSO(ComPtr<ID3D12Device> device) {
	D3D12_COMPUTE_PIPELINE_STATE_DESC motionBlurPSO = {};
	motionBlurPSO.pRootSignature = mRootSignature.Get();
	motionBlurPSO.CS =
	{
		reinterpret_cast<BYTE*>(mShaderCode->GetBufferPointer()),
		mShaderCode->GetBufferSize()
	};
	motionBlurPSO.Flags = D3D12_PIPELINE_STATE_FLAG_NONE;
	device->CreateComputePipelineState(&motionBlurPSO, IID_PPV_ARGS(mPSO.GetAddressOf()));
}
