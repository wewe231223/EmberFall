...
struct TreeCross_VOUT
{
    float4 position : SV_POSITION;
    float3 wPosition : POSITION1;
    float3 vPosition : POSITION2;
    float4 curPosition : POSITION3;
    float4 prevPosition : POSITION4;
...
TreeCross_VOUT TreeCross_VS(TreeCross_VIN input)
{
    ModelContext modelContext = modelContexts[input.instanceID];

    TreeCross_VOUT output;
...
Deffered_POUT TreeCross_PS(TreeCross_VOUT input)
{
    ...
    output.normal = float4(input.normal, 5.0f);
